#ifndef	__TASK_MSGID_H__
#define	__TASK_MSGID_H__

/***************************************************************/
/* MSGの定義は「REQ:奇数」「RES:偶数」となるように定義すること */
/***************************************************************/

/***** インクルード *****/

/***** 定数 *****/
enum EN_TASK_TASKID
{
	E_TASK_TASKID_MAIN = 0,
	/* GEN */
	E_TASK_TASKID_BT,
	E_TASK_TASKID_TM,
	E_TASK_TASKID_LD,
	/* APL */
	E_TASK_TASKID_LT,

	/* ここより上に定義すること */
	E_TASK_TASKID_NUM,
};

enum EN_MSGID_MAIN
{
	E_MSGID_MAIN_TEST_REQ =			( ( E_TASK_TASKID_MAIN << 24) | 0x00000001 ),
	E_MSGID_MAIN_TEST_RES,

	/* ここより上に定義すること */
	E_MSGID_MAIN_INVALID = 0x00000000,
};
/* GEN */
enum EN_MSGID_BT
{
	E_MSGID_BT_TEST_REQ =			( ( E_TASK_TASKID_BT << 24) | 0x00000001 ),
	E_MSGID_BT_TEST_RES,
	E_MSGID_BT_WUPCHK_REQ,
	E_MSGID_BT_WUPCHK_RES,
	E_MSGID_BT_STOP_REQ,
	E_MSGID_BT_STOP_RES,
	E_MSGID_BT_STACALIBRATION_REQ,
	E_MSGID_BT_STACALIBRATION_RES,
	E_MSGID_BT_CHGCALIBRATION_REQ,
	E_MSGID_BT_CHGCALIBRATION_RES,
	E_MSGID_BT_ENDCALIBRATION_REQ,
	E_MSGID_BT_ENDCALIBRATION_RES,
	E_MSGID_BT_REMOTESTART_REQ,
	E_MSGID_BT_REMOTESTART_RES,
	E_MSGID_BT_STARUNNING_REQ,
	E_MSGID_BT_STARUNNING_RES,
	/* ↑0xXX000010 */
	
	E_MSGID_BT_ENDRUNNING_REQ,
	E_MSGID_BT_ENDRUNNING_RES,
	E_MSGID_BT_SETCLIENTSEND_GYRO_REQ,
	E_MSGID_BT_SETCLIENTSEND_GYRO_RES,
	E_MSGID_BT_SETCLIENTSEND_COLOR_REQ,
	E_MSGID_BT_SETCLIENTSEND_COLOR_RES,
	E_MSGID_BT_SETLOG_LOGLAST_REQ,
	E_MSGID_BT_SETLOG_LOGLAST_RES,

	/* ここより上に定義すること */
	E_MSGID_BT_INVALID = 0x00000000,
};

enum EN_MSGID_TM
{
	E_MSGID_TM_TEST_REQ =			( ( E_TASK_TASKID_TM << 24) | 0x00000001 ),
	E_MSGID_TM_TEST_RES,

	/* ここより上に定義すること */
	E_MSGID_TM_INVALID = 0x00000000,
};

enum EN_MSGID_LD
{
	E_MSGID_LD_TEST_REQ =			( ( E_TASK_TASKID_LD << 24) | 0x00000001 ),
	E_MSGID_LD_TEST_RES,
	E_MSGID_LD_WUPCHK_REQ,
	E_MSGID_LD_WUPCHK_RES,
	E_MSGID_LD_STOP_REQ,
	E_MSGID_LD_STOP_RES,
	E_MSGID_LD_STARUNNING_REQ,
	E_MSGID_LD_STARUNNING_RES,
	E_MSGID_LD_ENDRUNNING_REQ,
	E_MSGID_LD_ENDRUNNING_RES,
	E_MSGID_LD_SETLOG_STATUSLOG_REQ,
	E_MSGID_LD_SETLOG_STATUSLOG_RES,
	E_MSGID_LD_SETLOG_CALIBRATELOG_REQ,
	E_MSGID_LD_SETLOG_CALIBRATELOG_RES,
	E_MSGID_LD_SETLOG_SYSTEMLOG_REQ,
	E_MSGID_LD_SETLOG_SYSTEMLOG_RES,
	/* ↑0xXX000010 */
	
	E_MSGID_LD_SETLOG_LOGLAST_REQ,
	E_MSGID_LD_SETLOG_LOGLAST_RES,
	E_MSGID_LD_STALOGDUMP_REQ,
	E_MSGID_LD_STALOGDUMP_RES,
	E_MSGID_LD_CHGLOGDUMP_REQ,
	E_MSGID_LD_CHGLOGDUMP_RES,
	E_MSGID_LD_ENDLOGDUMP_REQ,
	E_MSGID_LD_ENDLOGDUMP_RES,

	/* ここより上に定義すること */
	E_MSGID_LD_INVALID = 0x00000000,
};

/* APL */
enum EN_MSGID_LT
{
	E_MSGID_LT_TEST_REQ =			( ( E_TASK_TASKID_LT << 24) | 0x00000001 ),
	E_MSGID_LT_TEST_RES,
	E_MSGID_LT_TIMER_REQ,
	E_MSGID_LT_TIMER_RES,
	E_MSGID_LT_WUPCHK_REQ,
	E_MSGID_LT_WUPCHK_RES,
	E_MSGID_LT_SHUTDOWN_REQ,
	E_MSGID_LT_SHUTDOWN_RES,
	E_MSGID_LT_TOUCHBUTTON_REQ,
	E_MSGID_LT_TOUCHBUTTON_RES,
	E_MSGID_LT_BACKBUTTON_REQ,
	E_MSGID_LT_BACKBUTTON_RES,
	E_MSGID_LT_UPBUTTON_REQ,
	E_MSGID_LT_UPBUTTON_RES,
	E_MSGID_LT_DOWNBUTTON_REQ,
	E_MSGID_LT_DOWNBUTTON_RES,
	/* ↑0xXX000010 */
	
	E_MSGID_LT_LEFTBUTTON_REQ,
	E_MSGID_LT_LEFTBUTTON_RES,
	E_MSGID_LT_RIGHTBUTTON_REQ,
	E_MSGID_LT_RIGHTBUTTON_RES,
	E_MSGID_LT_CENTERBUTTON_REQ,
	E_MSGID_LT_CENTERBUTTON_RES,

	/* ここより上に定義すること */
	E_MSGID_LT_INVALID = 0x00000000,
};

#endif	/* __TASK_MSGID_H__ */
